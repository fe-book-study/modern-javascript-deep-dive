...
※ 클래스`constructor` vs 프로토타입`constructor` 클래스`constructor` | 프로토타입`constructor` :—|:— 메서드 | 프로퍼티 메서드로 해석되지 않고 클래스가 평가되어 생성한 함수 객체 코드의 일부가 된다. | 모든 프로토타입이 가지고 있는 프로퍼티, 생성자 함수를 가리킨다.
