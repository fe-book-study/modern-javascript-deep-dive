...
[16장 3.2 절의 접근자 프로퍼티](https://hong-p.github.io/javascript/javascript-deepdive-ch16/#3데이터-프로퍼티와-접근자-프로퍼티)에서 살펴본 것처럼 접근자 프로퍼티는 자체적인 값`[[Value]]`을 갖지 않고, 접근자 함수`[[Get]]`, `[[Set]]` 프로퍼티 어트리뷰트로 구성된 프로퍼티다.
...
[12.4.4절 Function 생성자 함수](https://hong-p.github.io/javascript/javascript-deepdive-ch12/#44-function-생성자-함수)에서 보았듯이 `Function`생성자 함수를 호출해 생성한 함수는 렉시컬 스코프를 만들지 않고 전역함수인 것처럼 스코프를 생성하며, 클로저도 만들지 않는다.
...
### 6.2 object 생성자 함수에 의해 성성된 객체의 프로토타입
...
### 11.2 객체 리터럴 내부에서 ____proto____에 의한 직접 상속
